...
class Config:
    ...
    USE_LMSA_MODULE = False     # Set False for Standard U-Net Baseline
    USE_MTL_HEAD    = False      # Set False for Single-Task Learning only
    ...
    USE_IMPLICIT_HESSIAN = True  # True  implicit curvature ablation
...
# ======================================================================================
# Implicit Curvature / Hessian Learning Block (Ablation for Reviewer 2 - Point 1)
# ======================================================================================
class ImplicitHessianBlock(nn.Module):
    """
    Implicit curvature learning via standard convolutions.
    This block allows the network to learn Hessian-like responses
    WITHOUT explicit vesselness or differential-geometry priors.
    """
    ...
    def forward(self, x):
        # Residual implicit feature refinement
        return x + self.block(x)
        
        

# ======================================================================================
# UNetLSAM_Boundary (WITH IMPLICIT vs EXPLICIT VESSELNESS ABLATION)
# ======================================================================================
class UNetLSAM_Boundary(nn.Module):
    """
    U-Net with:
    - Explicit LMSA vesselness (default)
    - OR Implicit curvature learning (ablation)
    - OR Plain U-Net (baseline)
    """
    ...
    def __init__(self, in_ch=1, out_ch=1, base=64,
        ...
        # -------------------------------------------------
        # Early-stage Enhancement Block (EXCLUSIVE)
        # -------------------------------------------------
        if self.use_lsam and not Config.USE_IMPLICIT_HESSIAN:
            # Explicit vesselness (LMSA)
            self.early_block = TopologyAwareLMSA(
                in_channels=in_ch,
                K=K,
                sigma_min=sigma_min,
                sigma_max=sigma_max,
                black_ridges=black_ridges,
                use_dropout_gate=lsam_use_dropout_gate,
        ...
        elif (not self.use_lsam) and Config.USE_IMPLICIT_HESSIAN:
            #  Implicit curvature learning (ablation)
            self.early_block = ImplicitHessianBlock(in_ch)

        else:
            #  Plain U-Net
            self.early_block = None
    ...
    def forward(self, x):
        #  Apply early enhancement (ONLY ONE active)
        if self.early_block is not None:
